...
| `gir-ef-2017.json` | verified (2026-09-02) | 4 / 4 (경유 · 도시가스LNG 각 tC + CO2 · GIR 2017년 승인 세분화값) |
| GIR_EF_2017 (sources.ts) | verified (2026-09-02) | 21개 대표값 note 에 하드코딩 (KEEI 학술지 표 17~ 열) |
...
## xlsm T2 EF 4건 불일치 원출처 추적 완료 (2026-09-02)

xlsm 이 T2 tC/CO2 컬럼에 넣은 값 중 4건 (등유·경유·항공유·도시가스LNG) 이 K-ETS 별표 12 (2025-04-11 고시 제2025-64호) · GIR 2022.1 공표 어느 것과도 일치하지 않았다. 학술 문헌 대조로 정체 확정:

### 근거 문헌

민선영·최용옥 (2024) 「월별 자료를 이용한 선도적인 에너지 분야 온실가스 배출량 산정」에너지경제연구 23(1):21-48 <표 2>
자료출처: 환경부 온실가스종합정보센터(2022); 2021년 승인 국가 온실가스 배출·흡수계수

### KEEI 표 헤더 (시대별 판)

`~06`(2006년까지 IPCC 기본) → `07~`(2007-2011 승인) → `12~`(2012-2016 승인) → **`17~`(2017년 승인, 2021년까지 5년 적용)** → `22~`(2022년 승인, GIR 2022.1 공표)

### 판정

- **경유(20.111) · 도시가스LNG(15.272)** = **GIR 2017년 승인 세분화값** 확정
  - 별표 12 표 B 는 배출권거래제 실무를 위해 등유·경유를 병합값(19.969)으로 · 천연가스·도시가스LNG 를 병합값(15.312)으로 축약
  - GIR 원본 공표는 두 연료를 세분화 (경유 20.111 / 도시가스 15.272)
  - xlsm 저자는 별표 12 대신 GIR 세분화 원본을 참조
  - → **verified 승격 완료** (`verified/gir-ef-2017.json` · 4개 엔트리)
- **등유(19.931) · 항공유(19.969)** = **xlsm 원본 오작성** 확정
  - xlsm 등유(19.931)=별표 12 항공유값, xlsm 항공유(19.969)=별표 12 등유값 → 저자가 두 값을 뒤바꿔 넣음
  - 문서(GIR_EF_2017)는 verified 이지만 이 두 값만 `maturity: "asserted"` 로 명시적 강등
  - `build_scope1_data.py` 의 `GIR_T2_TC_NOTES` / `GIR_T2_CO2_NOTES` 에서 `⚠` 프리픽스 warning note 주입
  - UI Cell 컴포넌트가 note 의 `⚠` 를 감지해 노란 warning 배지 (`⚠ 원출처 확인 요망`) 표시

감사자는 `sources.ts` 의 `GIR_EF_2017` note (2017년 승인 21개 대표값) 및 `GIR_EF_2022` note (2022.1 공표 25개 전체값) 와 대조해 정확한 국가 공표값을 확인할 수 있다.
