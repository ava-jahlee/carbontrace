...
```bash
npm test   # Vitest 파리티 8/8 + verified 58/58 PASS (450 measurements 승격 + xlsm 오류 4건 warning note)
```
...
| 배출계수 T2 (등유·경유·항공유·도시가스LNG) | GIR 이전 공표 판 추정 (등유·항공유는 별표 12 값 뒤바꿈 오류 · 경유·도시가스LNG는 이전 판 참조) | asserted + ⚠ note (UI 배지) |
...
- ✅ 2026-09-02 · xlsm T2 EF 불일치 4건 원출처 정밀 조사 완료 — 각 값에 `⚠` 프리픽스 warning note 주입, UI 에 노란 배지 표시
   - **등유·항공유 = xlsm 원본 오작성 확정**: xlsm 등유(19.931)=별표 12 항공유값, xlsm 항공유(19.969)=별표 12 등유값 → 저자가 두 값을 뒤바꿔 넣음. note 에 정확한 별표 12/GIR 2022.1 값과 정정 지침 명시
   - **경유·도시가스LNG = 원출처 불명**: 별표 12·GIR 2022.1 어느 값과도 일치 안 함. GIR 이전 공표(2011년대 초기 판) 참조 추정 → 이전 판 PDF 확보 후 재검토 예정
   - UI Cell 컴포넌트에 warning 배지 (⚠ 원출처 확인 요망) 구현 — asserted 상태 값 중 원본 오작성이 확인된 경우 감사자에게 즉시 노출
